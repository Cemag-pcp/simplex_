...
def tratar(input_csv):
    ...
    df = df.drop(columns={'Unnamed: 0', 'SEGUNDO PROCESSO'})
    df.rename(columns={'DESCRIÇÃO': 'codigo', 'MATÉRIA PRIMA': 'mp', 'Total': 'qtd_maxima', 'CONJUNTO': 'conjunto', 'COMPRIMENTO': 'tamanhos'}, inplace=True)
    ...
    df_agrouped = df.groupby(['conjunto', 'mp', 'codigo']).agg({
        'qtd_maxima': 'sum',
        'tamanhos': 'mean'
    }).reset_index()
...
def carregar_dados_csv(input_csv):
    ...
    df['conjunto'] = df['conjunto'].astype(str)
...
def otimizar_e_armazenar_resultados(df, resultados_pickle_path, resultados_csv_path):
    ordem_global = 1  # Variável para manter o controle das ordens globais
    resultados_totais = []  # Lista para armazenar todos os resultados

    # Agrupar o DataFrame por 'conjunto' e 'mp'
    grupos = df.groupby(['conjunto', 'mp'])

    # Iterar sobre cada grupo
    for (conjunto, mp), grupo in grupos:
        print(f"\nProcessando Conjunto: {conjunto}, MP: {mp}")
        ...
        while any(qtd > 0 for qtd in qtd_maxima):
            ...
            for i in range(len(codigos_pecas)):
                nome_var = f"{codigos_pecas[i]}_{conjunto}_{mp}_{ordem}"
            ...
            if prob.status != pulp.LpStatusOptimal:
                print(f"Não foi possível encontrar uma solução ótima na Ordem {ordem} para Conjunto {conjunto}, MP {mp}. Encerrando este grupo.")
                break
            ...
            print(f"Ordem {ordem} processada para Conjunto {conjunto}, MP {mp}. Tamanho total alocado: {total_tamanho_alocado_ordem}.")
            ordem += 1
...
excel_template_path = r'C:\Users\TI DEV\simplex_abdias\GERAR OP SERRA.xlsx'  # Substitua pelo caminho do seu template Excel
